...
# Importing SKU Sales Report File
...
# Importing Business Customer File
...
# Eliminating unnecessary variables from or_df
...
or_day_vec <- as.Date(or_num_df1$day, "%m/%d/%Y")
or_num_df1 <- mutate(or_num_df1, day = or_day_vec)

or_num_df1 <- mutate(or_num_df1, Orders = 1)
or_num_df1 <- or_num_df1[order(or_num_df1$Domain, or_num_df1$day),]

or_num_df1 <- ddply(or_num_df1, .(Domain), mutate, Order_Number = cumsum(Orders))
or_num_df1$Orders <- NULL

# Creating or_date_df
or_date_df <- or_num_df1
or_df2 <- or_date_df
...
or_df2$Domain <- tolower(or_df2$Domain)
traffic_source_df <- select(or_df2, Domain, Order_Number, traffic_source)
...
contact_df$Domain_Count
company_df$Associated.Contacts

summarise(company_df, max = max(Associated.Contacts))
...
company_df <- left_join(company_df, emails_del_df, by = "Domain")
...
company_df <- left_join(company_df, emails_open_df, by = "Domain")
...
company_df <- left_join(company_df, emails_clicked_df, by = "Domain")
...
contact_state_df$Contact_One
company_df <- left_join(company_df, contact_state_df, by = "Domain")

head(contact_state_df, n = 100L)
head(company_df$Domain, n = 100L)
str(contact_state_df)
...
company_df$Emails_Opened_Percent
company_df$Total_Emails_Delivered
...
company_df$Emails_Clicked_Percent



na_count <- sapply(company_df, function(y) sum(length(which(is.na(y)))))
na_count
company_df <- filter(company_df, Total_Emails_Delivered >= 0)
company_df$Total_Emails_Delivered

company_df$Industry <- tolower(company_df$Industry)



subdf <- company_df

subdf$Industry <- as.factor(subdf$Industry)
...
subdf$Contact_Count <- NULL

subdf$Order_One_Amount <- NULL
subdf$Contact_One <- NULL
...
subdf$Number.of.Visits <- NULL
subdf$Total_Emails_Opened <- NULL
subdf$Total_Emails_Clicked <- NULL
subdf$Emails_Opened_Percent <- NULL
subdf$Emails_Clicked_Percent <- NULL
subdf$Associated.Contacts <- NULL
subdf$Total_Emails_Delivered <- NULL
subdf$First_Order_Traffic_Source <- NULL

subdf1 <- filter(subdf, subdf$total_revenue > 2500)
...
# Total Revenue subdf
subdf$total_order_count <- NULL
lm.1 <- lm(total_revenue~. -Domain, data = subdf)
summary(lm.1)

df.ans <- predict(lm.1, data=subdf)
sqrt(mean((df.ans-subdf$total_revenue)^2))
summary(subdf$total_revenue)
